...
public class Main {
    ...
    void solve() throws IOException {
        ...
        // (1 - 1/m) / 2
        long probabilityGt = 1;
        long recipM = 1l * modInverseFermat(m, MOD);
        probabilityGt += MOD - recipM;
        probabilityGt *= modInverseFermat(2, MOD);
        probabilityGt %= MOD;

        long ans = 0;
        long cumConditional = 1;
        for (int i = 0; i < n; i++) {
            if (a[i] == 0 && b[i] == 0) {
                ans += cumConditional * probabilityGt % MOD;
                cumConditional = cumConditional * recipM % MOD;
            } else if (a[i] == 0) {
                long choices = m - b[i];
                ans += cumConditional * (choices * recipM % MOD) % MOD;
                cumConditional = cumConditional * recipM % MOD;
            } else if (b[i] == 0) {
                long choices = a[i] - 1;
                ans += cumConditional * (choices * recipM % MOD) % MOD;
